...
export function ModelDashboard({ model, modelKey, compressionResults, onNewResult, onClose }: ModelDashboardProps) {
  ...
  const allPollTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const mountedRef = useRef(true);

  const isReady = model.status === 'ready';

  const baselineResult = useMemo<DynamicResult>(() => ({
    strategy: 'baseline',
  ...
  const resultsWithBaseline = useMemo(() => {
    const nonBaselineResults = compressionResults.filter(
      (r) => (r.strategy || r.compression_method || '').toLowerCase() !== 'baseline'
    );
    return [baselineResult, ...nonBaselineResults];
  }, [baselineResult, compressionResults]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (allPollTimeoutRef.current) clearTimeout(allPollTimeoutRef.current);
    };
  }, []);

  useEffect(() => {
    setDataset(model.dataset || 'CIFAR10');
  }, [model.dataset]);
  ...
  const startPolling = useCallback(() => {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        console.log('[ModelDashboard] status update', {
          modelKey,
          running: s.running,
          step: s.step,
          detail: s.detail,
        ...
        if (!s.running) {
          ...
          if (s.error) {
            ...
            console.log('[ModelDashboard] compression completed', {
              modelKey,
  ...
  const handleCompress = async () => {
    ...
    try {
      console.log('[ModelDashboard] starting compression', { modelKey, method, dataset, epochs });
      await compressPreloaded(modelKey, method, dataset, epochs);
      startPolling();
    } catch (err: any) {
      console.error('[ModelDashboard] compression start failed', err);
  ...
  const handlePrepare = async () => {
    ...
    try {
      console.log('[ModelDashboard] triggering prepare', { modelKey });
      await triggerPrepare([modelKey]);
    } catch (err: any) {
      console.error('[ModelDashboard] preparation failed', err);
  ...
  const handleCompressAll = async () => {
    ...
    for (let i = 0; i < ALL_METHODS.length; ++i) {
      const currentMethod = ALL_METHODS[i];
      setAllStep(i);
      setAllCurrentMethod(currentMethod);
      setAllStatus(null);

      try {
        console.log('[ModelDashboard] compress-all start', {
          modelKey,
        ...
        await compressPreloaded(modelKey, currentMethod, dataset, epochs);
          ...
          const poll = async () => {
            try {
              ...
              if (!s.running) {
                ...
                if (s.result) {
                  console.log('[ModelDashboard] compress-all complete', {
                    modelKey,
              ...
              allPollTimeoutRef.current = setTimeout(poll, 1500);
            } catch (e: any) {
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 bg-green-100 rounded-xl flex items-center justify-center">
            <span className="text-green-700 font-bold text-lg">{model.model_name.charAt(0)}</span>
          </div>
          <div>
          ...
          {METHODS.map((m) => {
            ...
            return (
              ...
              >
                ...
                {hasResult && <span className="absolute top-1 right-1 w-2 h-2 rounded-full bg-green-400" />}
              </button>
            );
          })}
        </div>
        ...
        {allLoading && allCurrentMethod && (
          <div className="bg-white rounded-lg border border-green-200 p-4 mb-6">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-green-600 font-mono">{allStep + 1}/{ALL_METHODS.length}</span>
            </div>
            <div className="mb-2">
            ...
            {allStatus && allStatus.steps && allStatus.steps.length > 0 && (
              <div className="space-y-2">
                {allStatus.steps.map((step, idx) => {
                  const stepIndex = allStatus.steps.findIndex((s) => s.key === allStatus.step);
                  const isDone = idx < stepIndex || allStatus.step === 'complete';
                  const isActive = idx === stepIndex && allStatus.step !== 'complete';
                  return (
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <div className="space-y-3">
            {[...resultsWithBaseline].reverse().map((r, i) => {
              ...
              return <ResultCard key={i} result={r} methodLabel={methodLabel} accDiff={accDiff} />;
            })}
          </div>

          {resultsWithBaseline.length >= 2 && (
            <div className="mt-4 pt-4 border-t border-gray-100">
              <h4 className="text-xs font-medium text-gray-500 mb-2 uppercase tracking-wide">Method Comparison</h4>
          ...
          <div className="mt-2">
            ...
            <div className="w-full bg-gray-200 rounded-full h-1.5">
              <div
                ...
                style={{ width: `${Math.max(5, (result.size_MB / result.baseline_size_MB) * 100)}%` }}
              />
            </div>
          </div>
